...
static int  devQue_start(struct net_device *pD) {
    ...
    phyA=pP->s.phyTxBuf;
    for(i=0;i<LEN_QueTxDesc;i++) {
        pP->s.pQueTxDesc[i].f.bi=i;  /*Tx Buffer Index*/
        pP->s.pQueTxDesc[i].f.ba=phyA;  /*physical address of Tx Buf*/
        phyA+=(LEN_TxBuf+3)&~0x03;
    }/*for*/
    ...
    pBuf=pP->s.pTxBuf;
    for(i=0;i<LEN_QueTxDesc;i++) {
        pP->s.pTxBufDesc[i].vaddr=pBuf;  /*system address of buffer*/
        pP->s.pTxBufDesc[i].pFreeRtn=0;  /*no freeing after use*/
        pBuf+=(LEN_TxBuf+3)&~0x03;
    }/*for*/
