...
descriptor_error:
	...
	if (hub_configure(hub, endpoint) >= 0) {
		...
		if (!hdev->parent) {
			clear_port_feature(hdev, 2, USB_PORT_FEAT_POWER);
			clear_port_feature(hdev, 3, USB_PORT_FEAT_POWER);
		}
		return 0;
	}
#else
...
hub_ioctl(struct usb_interface *intf, unsigned int code, void *user_data)
{
	...
	case USBDEVFS_HUB_PORTCTRL: {
		...
		if (ctrl->value)
			...
			if (hdev->children[ctrl->port-1])
				usb_set_device_state(hdev->children[ctrl->port-1], USB_STATE_NOTATTACHED);
