...
static ssize_t snapspi_write(struct file *fp, const char __user *buf, size_t count, loff_t *ptr)
{
	...
	/* Write initial command, address and first 2 bytes */
	snapspi_chipenable();
	snapspi_sendbyte(CMD_AAIWR);
	snapspi_sendaddr(p);
	get_user(val, buf);
	buf++;
	...
	/* Write each pair of bytes with next command */
	for (total = 2; (total < count); total += 2) {
		snapspi_chipenable();
		snapspi_sendbyte(CMD_AAIWR);
		get_user(val, buf);
		buf++;
		...
		if ((total & 0x3fe) == 0x3fe)
			printk("\b\b\b\b\b\b\b\b%dk", (total + 1) >> 10);
	}
	printk("\n");
